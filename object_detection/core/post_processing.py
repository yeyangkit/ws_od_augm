      ...
      if use_static_shapes:
        ...
        per_image_boxes_3d = tf.reshape(
          tf.slice(per_image_boxes_3d, 3 * [0],
                   tf.stack([per_image_num_valid_boxes, -1, -1])), [-1, q, 6])
...
def multiclass_non_max_suppression_rpn(boxes,
                                       scores,
                                       score_thresh,
                                       iou_thresh,
                                       max_size_per_class,
...
def batch_multiclass_non_max_suppression_rpn(boxes,
                                             scores,
                                             score_thresh,
                                             iou_thresh,
                                             max_size_per_class,
  ...
  with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
    ...
    def _single_image_nms_fn(args):
      ...
      nmsed_boxlist, num_valid_nms_boxes = multiclass_non_max_suppression_rpn(
          per_image_boxes,
          per_image_scores,
          score_thresh,
          iou_thresh,
