...
from object_detection.utils import shape_utils
from object_detection.utils import visualization_utils
from object_detection.models.ssd_resnet_v1_fpn_feature_extractor import SSDResnet18V1FpnFeatureExtractor
from tensorflow import shape
...
class SSDAugmentationHybridSeqMetaArch(model.DetectionModel):
  ...
  def _concat_augm_feature_maps_with1x1(self, feature_maps, feature_maps_augm):
    concated_fm = []
    for i in range(4):
      transformed_fm = tf.layers.conv2d(feature_maps_augm[i],filters=128,kernel_size=3,padding='same',strides=1)
      output = self._multiResUnet_block_light(tf.concat([feature_maps[i], transformed_fm], axis=3), depth=128,
  ...
  def _concat_augm_preprocessed_inputs_STOPGRAD(self, prediction_dict, preprocessed_inputs):
  ...
  def _concat_augm_STOPGRAD(self, prediction_dict):
  ...
  def _hybridSeqExtraUnet_create_unet(self, x, f, kernel_size=2, stack_size=0, depth=4, training=True, outputs_channels=128):
  ...
  def _hybridSeqExtraUnet_extract(self, preprocessed_input, scope=None):

      # Create Unet
      pred_bels, pred_maps = self._hybridSeqExtraUnet_create_unet(preprocessed_input, training=True, depth=4, f=self._min_filter, kernel_size=3,
  ...
  def predict(self, preprocessed_inputs, true_image_shapes):  # features todo sep24 hestitate to use
    ...
    predictor_augm_dict = self._augm_predictor.predict(feature_maps_augm, preprocessed_inputs)
    augmented_maps = self._concat_augm_STOPGRAD(predictor_augm_dict)
    print(augmented_maps)
    ...
    #                                  not self._freeze_batchnorm),
    #                     updates_collections=batchnorm_updates_collections):
    #   with tf.variable_scope(None, 'FeatureExtractor_extra',
    #                            [augmented_maps]):
    #
    #     extra_feature_maps_augm = SSDResnet18V1FpnFeatureExtractor.extract_features(None, augmented_maps)
    #     print(extra_feature_maps_augm)
    #     feature_maps = self._concat_augm_feature_maps(extra_feature_maps_augm,feature_maps)

    extra_feature_maps_augm =   self._hybridSeqExtraUnet_create_unet(augmented_maps, f=16, kernel_size=2, stack_size=0, depth=4, training=True, outputs_channels=128)
    print(extra_feature_maps_augm)
    feature_maps = self._concat_augm_feature_maps_with1x1(feature_maps, extra_feature_maps_augm)
  ...
  def loss(self, prediction_dict, true_image_shapes, category_index, scope=None):
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      with tf.name_scope("augm_losses_and_weights"):
        ...
        augm_loss_belF = self._my_loss_L1(pred_bel_F, label_bel_F, xBiggerY=1.) * self._factor_loss_fused_bel_F
        augm_loss_belU = self._my_loss_L1(pred_bel_U, label_bel_U,
                                          xBiggerY=1.) * self._factor_loss_fused_bel_F / 10000
        tf.summary.scalar('augm_loss_belO', augm_loss_belO, family="custom_loss")
        tf.summary.scalar('augm_loss_belF', augm_loss_belF, family="custom_loss")
        tf.summary.scalar('augm_loss_belU', augm_loss_belU, family="custom_loss")
        # tf.summary.scalar('augm_metric_falsePositive', augm_loss_belO)
