...
class SSDAugmentationSharedEncoderMetaArch(model.DetectionModel):
    ...
    def _concat_augm_feature_maps(self, feature_maps, feature_maps_augm):
        concated_fm = []
        for i in range(4):
            output = self._multiResUnet_block_light(tf.concat([feature_maps[i], feature_maps_augm[i]], axis=3), depth=128, depth_out=128, name='bottleneck_merge_before_box_predictor{}'.format(i))
                ...
                with tf.variable_scope(None, self._extract_features_scope,
                    ...
                    shared_encoder_version
                      ...
                      (projection merged with upsampled topdown_features _ MultiResBlock _ output/finer topdown_features),  
                              ...
                              in augmentation branch are the projections and top_down feature maps CONCATED with each other
                    '''
                    shared_encoder_version = 'c_umMo_128_32_s'
                    ...
                    if shared_encoder_version == 'c_s32_c':
                        feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
                            preprocessed_inputs, 'c_s32_c')

                    if shared_encoder_version == 'c_s128_c':
                        feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
                            preprocessed_inputs, 'c_s128_c')


                    if shared_encoder_version == 's_s32_s':
                        feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
                            preprocessed_inputs, 's_s32_s')

                    if shared_encoder_version == 's_s128_s':
                        feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
                            preprocessed_inputs, 's_s128_s')


                    if shared_encoder_version == 'c_umMo_b32_c':
                        feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
                            preprocessed_inputs, "c_umMo_b32_c")


                    if shared_encoder_version == 'c_umMo_128_32_s':
                        feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
                            preprocessed_inputs, 'c_umMo_128_32_s')
    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        ...
        with tf.name_scope('Postprocessor'):
            ...
            detection_dict[
              fields.DetectionResultFields.detections_drivingCorridor_prediction] = prediction_dict['detections_drivingCorridor_prediction']
            detection_dict[
