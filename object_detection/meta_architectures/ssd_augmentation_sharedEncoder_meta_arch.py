...
class SSDAugmentationSharedEncoderMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    """
    super(SSDAugmentationSharedEncoderMetaArch, self).__init__(num_classes=box_predictor.num_classes)
  ...
  def _sum_augm_feature_maps(self, feature_maps, feature_maps_augm):
    summed_fm = []
    for i in range(4):
      summed_fm.append(feature_maps[i] + feature_maps_augm[i])
    return summed_fm

  def _sum_clone_augm_feature_maps(self, feature_maps, feature_maps_augm):
    summed_fm = []
    for i in range(4):
      summed_fm.append(feature_maps[i] + feature_maps_augm)
    return summed_fm
  ...
  def _conv_augm_pyramid(self, prediction_dict, feature_maps, conv_feature_maps_num):
    ...
    def _create_unet(x, f, kernel_size, stack_size, output_channel, depth, training):
      ...
      x = _conv_block(x, filters=f * (2 ** (depth - 1)), training=training, stack_size=stack_size, ksize=kernel_size,
                      name="augmented2features_deep")
    ...
    level_outputs = _create_unet(augm_map_dic, f=2, kernel_size=3, stack_size=1, output_channel=conv_feature_maps_num,
                                 depth=4, training=True)
  ...
  def _multiResUnet_block(self, x, depth, depth_out, name):
    """
  ...
  def _multiResUnet_block_light(self, x, depth, depth_out, name):
    ...
    # x4 = slim.conv2d(x4, depth, [1, 1], scope="multiRes_Block_{}_bottleneckOut".format(name))
    # x += x4
    x = slim.conv2d(x, depth_out, [3, 3], scope="multiRes_Block_{}_bottleneckEnd".format(name))
    return tf.nn.relu(x, name="multiRes_Block_{}_relu".format(name))

  def _concat_augm_feature_maps(self, feature_maps, feature_maps_augm):
    concated_fm = []
    for i in range(4):
      output = self._multiResUnet_block_light(tf.concat([feature_maps[i], feature_maps_augm[i]], axis=3), depth=128,
                                              depth_out=128, name='bottleneck_merge_before_box_predictor{}'.format(i))
  ...
  def predict(self, preprocessed_inputs, true_image_shapes):  # features todo sep24 hestitate to use
    ...
    """
    # prepr_inputs_det = tf.expand_dims(tf.expand_dims(preprocessed_inputs[0,:,:,0], axis=-1), axis=0)
    # prepr_inputs_obs =  tf.expand_dims(tf.expand_dims(preprocessed_inputs[0, :, :, 1], axis=-1), axis=0)
    # prepr_inputs_occ = tf.expand_dims(tf.expand_dims(preprocessed_inputs[0, :, :, 2], axis=-1), axis=0)
    ...
    if self._feature_extractor.is_keras_model:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        with tf.variable_scope(None, self._extract_features_scope,
                               [preprocessed_inputs]):
          '''
          shared_encoder_version
            1. c_s32_c: concated-symmetric structure for object detection and augmentation,
                    both with depth 128 96 48 32
                    which are CONCATED
                    in augmentation branch are the projections and top-down feature maps CONCATED with each other

            5. c_s128_c: concated_symmetric structure for object detection and augmentation,
                    both with depth 128 128 128 128
                    which are CONCATED
                    in augmentation branch are the projections and top_down feature maps CONCATED with each other
            ...
            s_s32_s: summed_symmetric structure for object detection and augmentation,
                    both with depth 128 96 48 32
                    which are SUMMED
                    in augmentation branch are the projections and top_down feature maps SUMMED with each other
                    MAY NEED MORE CONVs BEFORE BOX PREDICTOR

            2. s_s128_s: summed_symmetric structure for object detection and augmentation,
                    both with depth 128 128 128 128
                    which are SUMMED
                    in augmentation branch are the projections and top_down feature maps SUMMED with each other

            3. c_umMo_b32_c: concated_asymmetric structure for object detection and augmentation
            (projection merged with upsampled topdown_features _ MultiResBlock _ output/finer topdown_features),
                    both with depth 128 96 48 32
                    which are CONCATED
                    in augmentation branch are the projections and top_down feature maps CONCATED with each other

            4. c_umMo_128_32_s: concated_asymmetric structure for object detection and augmentation
            (projection merged with upsampled topdown_features _ MultiResBlock _ output/finer topdown_features),
                    both with depth 128 96 48 32
                    which are CONCATED
                    in augmentation branch are the projections and top_down feature maps CONCATED with each other
          '''
          shared_encoder_version = 'c_s128_c'

          feature_maps, feature_maps_augm = self._feature_extractor.extract_features_shared_encoder_for_augmentation(
            preprocessed_inputs, shared_encoder_version)
    ...
    # feature_maps = self._sum_augm_feature_maps(feature_maps, feature_maps_augm)
    ## feature_maps = multiResUnet_block(feature_maps, 128, name="bottleneck_before_box_predictor")

    sum_bel_f = False

    if shared_encoder_version == 'c_s32_c' or 'c_s128_c' or 'c_s128_c_attention' \
      or 'c_umMo_b32_c' or 'c_umMo_128_32_s' or 'c_umMo_128_32_s_attention':
      predictor_augm_dict = self._augm_predictor.predict(feature_maps_augm, preprocessed_inputs)
      feature_maps = self._concat_augm_feature_maps(feature_maps, feature_maps_augm)

    elif shared_encoder_version == 's_s32_s' or 's_s128_s':
      predictor_augm_dict = self._augm_predictor.predict(feature_maps_augm, preprocessed_inputs)
      feature_maps = self._sum_augm_feature_maps(feature_maps, feature_maps_augm)

    elif shared_encoder_version == 'c_s128_c' and sum_bel_f is True:
      predictor_augm_dict = self._augm_predictor.predict(feature_maps_augm, preprocessed_inputs)
      feature_maps = self._concat_augm_feature_maps(feature_maps, feature_maps_augm)
      feature_maps = self._sum_clone_augm_feature_maps(feature_maps, predictor_augm_dict['belief_F_prediction'])
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      detection_dict[
        fields.DetectionResultFields.z_min_detections_prediction] = prediction_dict['z_min_detections_prediction']
      detection_dict[
        fields.DetectionResultFields.z_max_detections_prediction] = prediction_dict['z_max_detections_prediction']
      detection_dict[
        fields.DetectionResultFields.z_min_observations_prediction] = prediction_dict['z_min_observations_prediction']
      detection_dict[
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      if self._use_uncertainty_weighting_loss:
        log_var_loc = tf.get_variable('log_variance_localization', dtype=tf.float32, trainable=True,
                                      initializer=0.0)
        localization_loss_new = 0.5 * localization_loss * tf.exp(-log_var_loc) + 0.5 * log_var_loc
        tf.summary.scalar('log_var_loc', log_var_loc, family="custom_loss")
        tf.summary.scalar('loc_loss_UNC_WEIGHT', localization_loss_new / localization_loss, family="custom_loss")
        tf.summary.scalar('augm_loc_loss_UNC_WEIGHTcoeff', tf.exp(-log_var_loc), family="custom_loss")
        ...
        classification_loss_new = classification_loss * tf.exp(-log_var_cls) + 0.5 * log_var_cls
        tf.summary.scalar('log_var_cls', log_var_cls, family="custom_loss")
        tf.summary.scalar('cls_loss_UNC_WEIGHT', classification_loss_new / classification_loss, family="custom_loss")
        tf.summary.scalar('augm_cls_loss_UNC_WEIGHTcoeff', tf.exp(-log_var_cls), family="custom_loss")
      ...
      label_bel_O_list = self.groundtruth_lists(fields.InputDataFields.groundtruth_bel_O)
      label_intensity = self.groundtruth_lists(fields.InputDataFields.groundtruth_intensity)
      # label_boxes_list = self.groundtruth_lists(fields.InputDataFields.groundtruth_boxes_mask)
      ...
      with tf.name_scope("augm_losses_and_weights"):
        ...
        if size_of_batch >= 2:
          for index in range(size_of_batch - 1):
            ...
            label_bel_O = tf.concat([label_bel_O,
                                    ...
                                    axis=0)
            # label_boxes = tf.concat([label_boxes, tf.expand_dims(label_boxes_list[index + 1], axis=0)],
            #                         axis=0)
        ...
        augm_loss_belU = self._my_loss_L1(pred_bel_U, label_bel_U, xBiggerY=1.) * self._factor_loss_fused_bel_F / 10000
        ...
        #              bel_o_mask[0, :, :, :], label_boxes[0], bel_cert_mask[0] * bel_o_mask[0] * label_boxes[0]), axis=1), 0)
        # tf.summary.image('1-belU___belCertMask___belOMask___bbox__result', bel_cert_mask_img)

        bel_cert_mask_img = tf.expand_dims(
          tf.concat(((1 - pred_bel_U)[0, :, :, :], tf.cast(bel_cert_mask[0], dtype=float),
                     bel_o_mask[0, :, :, :], bel_cert_mask[0] * bel_o_mask[0]), axis=1), 0)
        tf.summary.image('1-belU___belCertMask___belOMask__result', bel_cert_mask_img)

        augm_loss_zminObs = self._my_loss_L1(pred_z_min_observations, label_z_min_observations,
                                             weights=bel_o_mask,  # (bel_cert_mask+0.5)/1.5 rescale
        ...
        augm_loss_zmaxDet = self._my_loss_L1(pred_z_max_detections, label_z_max_detections,
                                             weights=bel_cert_mask * bel_o_mask,
      ...
      if self._use_uncertainty_weighting_loss:
        log_var_augm = tf.get_variable('log_variance_augmentation', dtype=tf.float32, trainable=True,
                                       initializer=0.0)
        augm_loss_new = 0.5 * augm_loss * tf.exp(-log_var_augm) + 0.5 * log_var_augm
        tf.summary.scalar('log_var_augm', log_var_augm, family="custom_loss")
        tf.summary.scalar('augm_loss_UNC_WEIGHT', augm_loss_new / augm_loss, family="custom_loss")
        tf.summary.scalar('augm_loss_UNC_WEIGHTcoeff', tf.exp(-log_var_augm), family="custom_loss")
        augm_loss = augm_loss_new
