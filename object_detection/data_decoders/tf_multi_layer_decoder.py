...
class MultilayerImages(slim_example_decoder.ItemHandler):
    ...
    def tensors_to_item(self, keys_to_tensors):
        ...
        for idx in range(1, len(self._image_keys)):
            image_tensor_layer = self._decode(keys_to_tensors[self._image_keys[idx]], self._layer_channels[idx],
                                              image_format)
...
class TfMultiLayerDecoder(data_decoder.DataDecoder):
    ...
    def decode(self, tf_example_string_tensor):
        ...
        print("shape is () = " + str(tensor_dict[fields.InputDataFields.groundtruth_bel_O].shape))n
