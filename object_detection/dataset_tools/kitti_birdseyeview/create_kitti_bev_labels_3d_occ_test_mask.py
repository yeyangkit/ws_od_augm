...
def create_kitti_labels(output_path, label_map_path, calib_dir, image_dir, image_ground_dir, mask_dir, graph_dir, examples):
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            for idx, example in enumerate(examples):
                ...
                image_vis = cv2.imread(image_path_prob)
                ...
                image_stacked = np.stack([image_hits, image_occ, image_obs, image_int, image_zmin, image_zmax], axis=-1)
...
def main(_):
    ...
    test_list = ["%06d" % idx for idx in range(0, 7518)]
    ...
    create_kitti_labels(val_output_path, label_map_path, calib_dir, data_dir, data_ground_dir, data_mask, graph_dir, test_list)
