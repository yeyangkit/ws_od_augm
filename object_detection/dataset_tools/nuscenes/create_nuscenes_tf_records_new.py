...
def _count_number_detections(path, prefix, box_aligned, p):
    scale = 255 / p['batch_processor']['detections']['max']
    x_min, y_min, x_max, y_max = box_aligned
    fn = os.path.join(path, prefix + '_detections_cartesian.png')
    image_det = cv2.imread(fn, 0)
    height, width = image_det.shape
    x_min = x_min * width
    x_max = x_max * width
    y_min = y_min * height
    y_max = y_max * height
    image_det_box = image_det[int(y_min):math.ceil(y_max),
                    int(x_min):math.ceil(x_max)]
    return round(image_det_box.sum() / scale)
...
def dict_to_tf_example(labels_corners,
  ...
  for idx, label_corner in enumerate(labels_corners):
    ...
    num_detections = _count_number_detections(image_dir, image_prefix, (x_min, y_min, x_max, y_max), params)
    if num_detections == 0:
        continue
    xmin.append(x_min)
    ymin.append(y_min)
