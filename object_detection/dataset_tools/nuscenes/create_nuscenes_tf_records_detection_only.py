...
flags.DEFINE_boolean('vis_result', False, 'Whether to visualize the result.')
...
def dict_to_tf_example(labels_corners,
  ...
  if FLAGS.range is not None:
    width = round(FLAGS.range / params['pointcloud_grid_map_interface']['grids']['cartesian']['resolution']['y'])
    height = round(FLAGS.range / params['pointcloud_grid_map_interface']['grids']['cartesian']['resolution']['x'])
  xmin = []
  ymin = []
  xmax = []
  ymax = []
  ...
  for idx, label_corner in enumerate(labels_corners):
    ...
    angle_rad = - labels_data[idx].rz - math.pi / 2
    angle_rad = _flipAngle(angle_rad)
...
def create_tf_record(fn_out, split, vis_results):
  ...
  for i, sample_token in enumerate(sample_tokens):
      ...
      print('Finished writing ', i, '-th sample.')
      if (vis_results):
          print('Visualising')
...
def visualize_results(dir,
  ...
  img_output_vis = os.path.join('/home/ding/test', os.path.basename(img_name))
