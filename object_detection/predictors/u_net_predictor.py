...
tf.app.flags.DEFINE_integer('image_size',
                            512,
                            """Images will be scaled to this size before they get passed to nn.""")
# Model params

tf.app.flags.DEFINE_integer('kernel_size', 3, """kernel_size""")

tf.app.flags.DEFINE_integer('filters', 8, """filters after first convolution""")

tf.app.flags.DEFINE_integer('stack_size', 3, """number of convolutions stacked""")

tf.app.flags.DEFINE_integer('depth', 2, """number of pooling ops""")



tf.app.flags.DEFINE_boolean('layer_norm', True, """Whether do batch norm or don't""")

# Train params

tf.app.flags.DEFINE_float("lr", 1e-5, """initial learning rate for adam""")
...
tf.app.flags.DEFINE_integer('epochs',
                            400,
                            """Number of train/eval iterations.""")

import datetime

now = datetime.datetime.now()
tf.app.flags.DEFINE_string('time', now.strftime("%Y-%m-%d-%H-%M-%S"), """ current time """)  # bug todo



tf.app.flags.DEFINE_string('logdir',
                           "/mrtstorage/users/students/yeyang/runs_JulAug/Unet_tf/11_29_1306_lossweight_isL1xF10xO10xW10_ck_is{}_f_is{}_s_is{}_d_is{}_ln_is{}_lr_is{}_bs_is{}_epochs_is{}_{}".format(
                               FLAGS.kernel_size, FLAGS.filters, FLAGS.stack_size, FLAGS.depth, FLAGS.layer_norm,
                               '{:.0e}'.format(FLAGS.lr), FLAGS.batch_size, FLAGS.epochs, FLAGS.time),
                           """Logging path""")

tf.app.flags.DEFINE_integer('save_checkpoints_secs', 240,
                            """Number of seconds between the creation of model checkpoints.
                            Pass 0 to disable checkpoints""")

tf.app.flags.DEFINE_integer('save_summary_steps',
                            100,
                            """Number of steps between the computation of summaries for the event log.
...
class UNetPredictor(flow_predictor.FlowPredictor):  ## todo fragen relationship between od and augm where to see
  ...
  def _create_unet(self, x, outputs_channels, training):
      ...
      for i in reversed(range(FLAGS.depth+1)): ## todo fragen
