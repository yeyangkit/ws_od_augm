...
class SharedEncoderPredictor(beliefs_predictor.BeliefPredictor):  #
    ...
    def _multiResUnet_block(self, x, depth, name, stack_size=1):
        ...
        for i in range(stack_size):
            x = tf.layers.conv2d(x, depth, [1, 1], name="multiRes_Block_{}_{}_bottleneckIn".format(name, i))
            x1 = tf.layers.conv2d(x, depth, [3, 3], name="multiRes_Block_{}_{}_inception3x3conv".format(name, i),
                                  padding='same')
            x2 = tf.layers.conv2d(x1, depth, [3, 3], name="multiRes_Block_{}_{}_inception5x5conv".format(name, i),
                                  padding='same')
            x3 = tf.layers.conv2d(x2, depth, [3, 3], name="multiRes_Block_{}_{}_inception7x7conv".format(name, i),
                                  padding='same')
            x4 = tf.concat((x1, x2, x3), axis=3, name="multiRes_Block_{}_{}_concat".format(name, i))
            x4 = tf.layers.conv2d(x4, depth, [1, 1], name="multiRes_Block_{}_{}_bottleneckOut".format(name, i))
            x += x4
    ...
    def _multiResUnet_resPath(self, x, depth, name, stack_size=1):
        ...
        """
        for i in range(stack_size):
            x1 = tf.layers.conv2d(x, depth, [3, 3], name="multiRes_shortcut_{}_{}_3x3conv".format(name, i),
                                  padding='same')
            x2 = tf.layers.conv2d(x, depth, [1, 1], name="multiRes_shortcut_{}_{}_1x1conv".format(name, i))
            x = x1 + x2
        return x
    ...
    def _create_input_conv_net(self, preprocessed_input):
        # x = self._multiResUnet_resPath(preprocessed_input, depth=int(self._detectionFM_filters), stack_size=1, name='preprocessed_input')
        x = tf.layers.conv2d(preprocessed_input, filters=self._detectionFM_filters / 4, kernel_size=1, strides=1,
                             padding='same',
                             name='preprocessed_input_short_cut')
        return x  # tf.nn.relu(x)

    # def _create_unet_end(self, last_feature_maps_augm, short_cut, bels_outputs_channels, maps_outputs_channels):
    #     x = tf.layers.conv2d_transpose(last_feature_maps_augm, filters=int(self._detectionFM_filters / 4),
    #                                    kernel_size=3, strides=2,
    #                                    padding='same', name='augm_transpose')
    #
    #     x = tf.concat([x, short_cut], 3, name='concate_original_dim')
    #
    #     x = self._multiResUnet_block(x, depth=int(self._detectionFM_filters / 4), stack_size=self._stack_size,
    #                                   name="augm_publicHead_multiResUnet_block")
    #
    #
    #     x1 = self._multiResUnet_block(x, depth=int(self._detectionFM_filters / 8), stack_size=self._stack_size,
    #                                   name="augm_BELS_multiResUnet_block_after_transpose")
    #
    #     with tf.variable_scope("augm_BELS_end"):
    ...
    #         bels = tf.nn.softmax(x1)
    #
    #     x2 = self._multiResUnet_block(x, depth=int(self._detectionFM_filters / 8), stack_size=self._stack_size,
    #                                   name="augm_MAPS_multiResUnet_block_after_transpose")
    #
    #     with tf.variable_scope("augm_MAPS_end"):
    ...
    #
    def _create_unet_end(self, last_feature_maps_augm, short_cut, bels_outputs_channels, maps_outputs_channels):
        x = tf.layers.conv2d(last_feature_maps_augm, filters=int(self._detectionFM_filters / 2), kernel_size=1, strides=1,
                                   name='last_augm_fm_before_transpose')
        x = tf.layers.conv2d_transpose(x, filters=int(self._detectionFM_filters / 4),
                                       kernel_size=3, strides=2,
                                       padding='same', name='augm_transpose')
